...
class AppView {
  ...
  renderStatsHoy(rows) {
    ...
    const getEmojiForExam = (nombre) => {
      const n = nombre.toLowerCase();
      // Heces / materia fecal / coprológico
      if (/heces|fecal|coprol[oó]g|materia fecal|copropar|huevo.*par[aá]sito|par[aá]sito.*hec|popo|excremento/.test(n))
        return '💩';
      // Sangre / hemograma / hematología
      if (/sangr[e]?|hemo|hemat|eritro|leuco|plaqueta|gl[oó]bulo|coagul|biometr[ií]a|hemograma|hematocrit|hemoglobin|ferritin|hierro|sanguí/.test(n))
        return '🩸';
      // Orina / uroanálisis
      if (/orin[a]?|urin|uroan[aá]lisis|urocult/.test(n))
        return '🧫';
      // Cultivo / microbiología
      if (/cultiv|bacteri|microbi/.test(n))
        return '🦠';
      // Hormona / tiroides / metabólico
      if (/hormon|tiroides|tsh|t3\b|t4\b|estrogen|progesteron|testosteron|insulin|glucosa|glicemia/.test(n))
        return '⚗️';
      // Imagen / rayos X / ultrasonido
      if (/rayos[\s-]?x|ultrason|ecograf|tomograf|resonan|mamogra/.test(n))
        return '🩻';
      // Sin categoría especial → usa el array rotativo
      return null;
    };
    rows.forEach((r, i) => {
      const emoji = getEmojiForExam(r.nombre) ?? defaultEmojis[i % defaultEmojis.length];
